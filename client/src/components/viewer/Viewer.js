    ...
    <Canvas dpr={[1, 2]} onPointerMissed={() => setTarget(null)} camera={{ position: [-1, 1, 5], fov: 50 }}>
...

      {
        transforms.map((trans) => (
          ...
          (() => {
            if (trans.objnum === 1 && (selectedScene?.googleId === trans?.creator || selectedScene?.creator === trans?.creator) && selectedScene?.sceneId === trans?.sceneInfomationId) {
            ...
            else if (trans.objnum === 2 && (selectedScene?.googleId === trans?.creator || selectedScene?.creator === trans?.creator) && selectedScene?.sceneId === trans?.sceneInfomationId) {
